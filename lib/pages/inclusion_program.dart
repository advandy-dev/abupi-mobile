...
class InclusionProgramScreen extends StatefulWidget {
  const InclusionProgramScreen({super.key});

  @override
  State<InclusionProgramScreen> createState() => _InclusionProgramScreenState();
}

class _InclusionProgramScreenState extends State<InclusionProgramScreen> {
  Map<String, dynamic>? _pageData;
  String _title = 'Inklusi';
  ...
  String _getSlugForLanguage(String language) {
    return language == 'en' ? 'work-program-inclusion' : 'program-kerja-inklusi';
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonList.isNotEmpty) {
          ...
          setState(() {
            _pageData = data;
            _title = data['acf']['strategic_title'] ?? 'Inklusi';
          });
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final imageUrl = (acfImage != null && acfImage.toString().isNotEmpty)
        ? acfImage.toString()
        : 'https://placehold.co/600x400/png?text=ABUPI+Inklusi';
