...
class AccelerationProgramScreen extends StatefulWidget {
  const AccelerationProgramScreen({super.key});

  @override
  State<AccelerationProgramScreen> createState() => _AccelerationProgramScreenState();
}

class _AccelerationProgramScreenState extends State<AccelerationProgramScreen> {
  Map<String, dynamic>? _pageData;
  String _title = 'Akselerasi';
  ...
  String _getSlugForLanguage(String language) {
    return language == 'en' ? 'work-program-acceleration' : 'program-kerja-akselerasi';
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonList.isNotEmpty) {
          ...
          setState(() {
            _pageData = data;
            _title = data['acf']['strategic_title'] ?? 'Akselerasi';
          });
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final imageUrl = (acfImage != null && acfImage.toString().isNotEmpty)
        ? acfImage.toString()
        : 'https://placehold.co/600x400/png?text=ABUPI+Akselerasi';
    ...
    // Extract Work Programs (work_program_1, work_program_2, ...)
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _title.isEmpty ? 'Akselerasi' : _title,
