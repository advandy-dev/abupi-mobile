...
class SustainabilityProgramScreen extends StatefulWidget {
  const SustainabilityProgramScreen({super.key});

  @override
  State<SustainabilityProgramScreen> createState() => _SustainabilityProgramScreenState();
}

class _SustainabilityProgramScreenState extends State<SustainabilityProgramScreen> {
  Map<String, dynamic>? _pageData;
  String _title = 'Keberlanjutan';
  ...
  String _getSlugForLanguage(String language) {
    return language == 'en' ? 'work-program-sustainability' : 'program-kerja-keberlanjutan';
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonList.isNotEmpty) {
          ...
          setState(() {
            _pageData = data;
            _title = data['acf']['strategic_title'] ?? 'Keberlanjutan';
          });
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final imageUrl = (acfImage != null && acfImage.toString().isNotEmpty)
        ? acfImage.toString()
        : 'https://placehold.co/600x400/png?text=ABUPI+Keberlanjutan';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _title.isEmpty ? 'Keberlanjutan' : _title,
